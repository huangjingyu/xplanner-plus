...
<xplanner:content>
   ...
   <!-- <xplanner:navigation type="net.sf.xplanner.domain.Person" back="true"/> --> 

<span class="title"><h1><bean:message key="person.label.name"/> <a href="">${person.name}</a> </h1></span>
<%--
<span class="title"><bean:message key="person.label.contact_info"/></span>
<blockquote>
  <p><bean:message key="person.label.email"/> <a href="mailto:${person.email}">${person.email}</a><br>
     <bean:message key="person.label.phone"/> ${person.phone}</p>
</blockquote> --%>
